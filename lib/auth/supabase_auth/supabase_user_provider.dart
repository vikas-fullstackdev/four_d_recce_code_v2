...
class FourDRecceCodeSupabaseUser extends BaseAuthUser {
  FourDRecceCodeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fourDRecceCodeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FourDRecceCodeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
